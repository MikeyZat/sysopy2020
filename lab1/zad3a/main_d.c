...
{
  arbitrary f_create_files_tab;
  *(void **)(&f_create_files_tab) = dlsym(dl_handle, "create_files_tab");

  int size = (int)strtol(size_as_string, NULL, 10);
  blocks_array *new_blocks_tab = f_create_files_tab(size);
  return new_blocks_tab;
}

void exec_compare_pairs(char *files_as_chars)
{
  arbitrary f_create_files_sequence, f_calculate_diff;
  *(void **)(&f_create_files_sequence) = dlsym(dl_handle, "create_files_sequence");
  *(void **)(&f_calculate_diff) = dlsym(dl_handle, "calculate_diff");
  ...
  f_calculate_diff(f_create_files_sequence(files_str));
}

int exec_create_blocks(char *file_name, blocks_array *blocks_tab)
{
  arbitrary f_create_operations_blocks;
  *(void **)(&f_create_operations_blocks) = dlsym(dl_handle, "create_operations_blocks");
  return (int)f_create_operations_blocks(file_name, blocks_tab);
}

void exec_remove_block(char *index_as_str, blocks_array *blocks_tab)
{
  arbitrary f_delete_file_block;
  *(void **)(&f_delete_file_block) = dlsym(dl_handle, "delete_file_block");

  int index = atoi(index_as_str);
  f_delete_file_block(index, blocks_tab);
}

void exec_remove_operation(
    char *block_index_as_str, char *operation_index_as_str, blocks_array *blocks_tab)
{
  arbitrary f_delete_operation_block;
  *(void **)(&f_delete_operation_block) = dlsym(dl_handle, "delete_operation_block");
  ...
  f_delete_operation_block(op_index, curr_block);
}
